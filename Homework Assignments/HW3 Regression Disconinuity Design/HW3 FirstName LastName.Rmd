...
2. Load in the data as `df`
3. Create the variables in `df`
...
4. Create the data frame `df_reg` such that `lagdemovoteshare` $\in (0.48, 0.52)$
...
## Continuity Assumption: Running Variable Density Plot
**********************************************************************
...
1. create a data frame `df_hist` from `df` by
...
Do you see evidence of bunching/cutoff manipulation? How do you know?
...
1. create the data frame `df_plot` from `df` by
    ...
    - `summarise()` `across()` the 
        ...
        - first stage variable `lagdemocrat` (whether a deomcrat won)
...
Based upon the figure, are you concerned about imbalance on any of the variables? Why?
...
3. run four regressions at once saved as `fit_x_bal` by 
...
4. Produce an `etable()` with `fit_x_bal` by 
...
1. Produce a figure using the same arguments as the covariate balance figure, except for use the three outcome variables `score`, `lagscore`, and `democrate`, and set the `ncol` argument of `plot_grid` equal to 3.
...
2. Using the **same setup** as the covariate balance tests
...
Print your *coefficient* estimates for column (4) and column (5) by using `fit_outcomes` and `coef()`
...
3. Assuming the standard errors of the original paper apply to our estimates on "elect" and "effect", do voters elect, affect, or both elect and affect policies?
