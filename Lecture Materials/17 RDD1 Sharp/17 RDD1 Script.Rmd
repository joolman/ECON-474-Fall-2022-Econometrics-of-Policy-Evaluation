...
fit_left = lm(freq ~ poly(dist_from_cut_bin, degree_n), data = df_plot, subset = df_plot$D == 1)
x_left = seq(-1.2, 0, length = 100)
df_left = data.frame(dist_from_cut_bin = x_left)
df_left$freq_hat = predict(fit_left, df_left)


fit_right = lm(freq ~ poly(dist_from_cut_bin, degree_n), data = df_plot, subset = df_plot$D == 0)
x_right = x_left*-1
df_right = data.frame(dist_from_cut_bin = x_right)
df_right$freq_hat = predict(fit_right, df_right)


ggplot(df_plot, aes(x = dist_from_cut_bin, y = freq)) +
  geom_point(shape = 1, size = 3) +
  geom_line(data = df_left, aes(x = dist_from_cut_bin, y = freq_hat)) +
  geom_line(data = df_right, aes(x = dist_from_cut_bin, y = freq_hat))
```

The last step is to verify that we do not have a statistically significant coefficient for the cutoff value.


```{r density reg}
feols(freq ~ poly(dist_from_cut_bin, 2)*D, data = df_plot)
```
...
fit_left = lm(probation_year1 ~ poly(dist_from_cut_bin, degree_n), data = df_plot, subset = df_plot$D == 1)
x_left = seq(-1.3, 0, length = 100)
df_left = data.frame(dist_from_cut_bin = x_left)
df_left$probation_year1_hat = predict(fit_left, df_left)


fit_right = lm(probation_year1 ~ poly(dist_from_cut_bin, degree_n), data = df_plot, subset = df_plot$D == 0)
x_right = x_left*-1
df_right = data.frame(dist_from_cut_bin = x_left*-1)
df_right$probation_year1_hat = predict(fit_right, df_right)


ggplot(df_plot, aes(x = dist_from_cut_bin, y = probation_year1)) +
  geom_point(shape = 1, size = 3) +
  geom_line(data = df_left, aes(x = dist_from_cut_bin, y = probation_year1_hat)) +
  geom_line(data = df_right, aes(x = dist_from_cut_bin, y = probation_year1_hat))
```
...
for(s in 1:length(subsets)){
  ...
  fit_left = lm(left_school ~ poly(dist_from_cut_bin, degree_n), data = df_plot, subset = df_plot$D == 1)
  x_left = seq(-1.3, 0, length = 100)
  df_left = data.frame(dist_from_cut_bin = x_left)
  df_left$left_school_hat = predict(fit_left, df_left)
  
  
  fit_right = lm(left_school ~ poly(dist_from_cut_bin, degree_n), data = df_plot, subset = df_plot$D == 0)
  x_right = x_left*-1
  df_right = data.frame(dist_from_cut_bin = x_left*-1)
  df_right$left_school_hat = predict(fit_right, df_right)
  
  
  fig3[[s]] = ggplot(df_plot, aes(x = dist_from_cut_bin, y = left_school)) +
    geom_point(shape = 1, size = 3) +
    geom_line(data = df_left, aes(x = dist_from_cut_bin, y = left_school_hat)) +
    geom_line(data = df_right, aes(x = dist_from_cut_bin, y = left_school_hat)) +
...
fit_left = lm(nextGPA ~ poly(dist_from_cut_bin, degree_n), data = df_plot, subset = df_plot$D == 1)
x_left = seq(-1.3, 0, length = 100)
df_left = data.frame(dist_from_cut_bin = x_left)
df_left$nextGPA_hat = predict(fit_left, df_left)


fit_right = lm(nextGPA ~ poly(dist_from_cut_bin, degree_n), data = df_plot, subset = df_plot$D == 0)
x_right = x_left*-1
df_right = data.frame(dist_from_cut_bin = x_left*-1)
df_right$nextGPA_hat = predict(fit_right, df_right)


ggplot(df_plot, aes(x = dist_from_cut_bin, y = nextGPA)) +
  geom_point(shape = 1, size = 3) +
  geom_line(data = df_left, aes(x = dist_from_cut_bin, y = nextGPA_hat)) +
  geom_line(data = df_right, aes(x = dist_from_cut_bin, y = nextGPA_hat))
```
