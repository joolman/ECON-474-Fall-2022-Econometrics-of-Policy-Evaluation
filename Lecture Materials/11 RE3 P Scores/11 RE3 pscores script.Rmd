---
title: "11 RE3 Approximate Matching: Propensity Scores"
...
**Caution**: I would only recommend creating a control group from an entirely different *data source* as a method of last resort. 
...
# Loading in the National Supported Work
...
# Loading in the Current Population Survey data
...
$$E[\delta] = E[Y^1 | D = 1] - E[Y^0 | D = 0] = E[Y^1] - E[Y^0]$$
...
This is entirely due to selection bias because: $E[Y^1 | D = 0] \neq E[Y^0 | D = 0]$.
...
```{r pscore}
df = df %>%
  mutate(pscore = fit_pscore$fitted.values)
```
...
fit_nn = lm(re78 ~ 
              treat +
              poly(age, 3) + educ*re74 + I(educ^2) + re75 + black + hisp + not_emp_74 + not_emp_75,
                data = df_m)
summary(fit_nn)
...
```{r outlier pscores}
df_m %>%
  group_by(Group) %>% # could use `treat` too
