---
title: "15 IV3 Local Average Treatment Effects"
...
card = read_csv('C:/Users/johnj/Dropbox/Julian/Academia/UIUC/Semester 11 Fall 2022/ECON474/Data/card.csv')
head(card)
```
...
```{r etable}
fit_ols = feols(lwage ~ educ + exper + expersq + black + south + smsa,
                 data = card, se = 'hetero')

fit_rf = feols(lwage ~ nearc4 + exper + expersq + black + south + smsa,
               data = card, se = 'hetero')

fit_fs = feols(educ ~ nearc4 + exper + expersq + black + south + smsa,
               data = card, se = 'hetero')

fit_2sls = feols(lwage ~  exper + expersq + black + south + smsa | educ ~ nearc4,
               data = card, se = 'hetero')

etable(fit_ols, fit_rf, fit_fs, fit_2sls,
       fitstat = c('n', 'r2', 'f'),
       se.below = TRUE,
       order = c('Intercept', 'educ', 'nearc4'))
...
Does the 2SLS least squares return this result?
