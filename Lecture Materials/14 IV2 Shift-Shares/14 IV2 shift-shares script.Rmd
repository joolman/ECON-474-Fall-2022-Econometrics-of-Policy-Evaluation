...
usa = fread('C:/Users/johnj/Dropbox/Julian/Academia/UIUC/Semester 11 Fall 2022/ECON474/Data/shift_shares.csv')
...
fwrite(ss, 'C:/Users/johnj/Dropbox/Julian/Academia/UIUC/Semester 11 Fall 2022/ECON474/Data/shift_shares cleaned.csv')
```

Now, to load in the data:

```{r cleaned}
ss = fread('C:/Users/johnj/Dropbox/Julian/Academia/UIUC/Semester 11 Fall 2022/ECON474/Data/shift_shares cleaned.csv')
...
df = ss %>%
  ...
  summarise(wage_at = weighted.mean(incwage, perwt),
            emp_at = sum(perwt)) # the data are trimmed to be only employed people so this line is okay
head(df)
```
...
emp_agg = ss %>%
  filter(ind1990 %in% c(10, 11, 31:392, 500:711, 721, 732, 800,
                        841, 882:892)) %>%
...
emp_t_shift = emp_t_shift %>%
  ...
  mutate(shift = log(emp_atilde_jt) - log(emp_atilde_jt_lag)) %>%
...
```{r IV}
iv = left_join(emp_t_shift, emp_t0_share) %>%
  na.omit() %>%
...
```{r OLS}
fit_ols = feols(log(wage_at) ~ log(emp_at) | metarea, df, se = 'hetero')
```

2. Estimate the reduced form, and store it as an object.

```{r reduced form}
fit_rf = feols(log(wage_at) ~ shift_share | metarea, df, se = 'hetero')
```

3. Estimate the first stage, and store it as an object.

```{r first stage}
fit_fs =feols(log(emp_at) ~ shift_share | metarea, df, se = 'hetero')
```
