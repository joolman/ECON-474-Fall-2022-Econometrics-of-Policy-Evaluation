---
title: "21 DiD3 Differential Timing Difference-in-Differences"
...
castle = read_csv("C:/Users/johnj/Dropbox/Julian/Academia/UIUC/Semester 11 Fall 2022/ECON474/Data/DiD3 castle.csv")
...
df = castle %>%
  filter(treated == 1 &
           cdl < 1) %>%
  group_by(state) %>%
  summarise(reform_year = tail(year, 1)) %>%
  full_join(castle) %>%
  mutate(event_time = year - reform_year + 0,
         event_time = ifelse(is.na(event_time), 0, event_time),
         event_time = factor(event_time),
         event_time = fct_relevel(event_time, "0")) 
```
...
```{r event reg}
fit_event = feols(l_homicide ~ event_time | state + year,
      data = df,
      weights = df$popwt) %>% summary
```
...
```{r event df plot}
# coef(fit_event)
# str_sub(names(coef(fit_event)), 11, -1)
# as.numeric(str_sub(names(coef(fit_event)), 11, -1))

df_event =
  cbind(coef(fit_event),
      confint(fit_event, cluster = 'state')) %>% 
  mutate(event_time = as.numeric(str_sub(names(coef(fit_event)), 11, -1))) %>%
  rbind(c(0, 0, 0, 0))
```
...
```{r event plot}
ggplot(df_event, aes(x = event_time, y = `coef(fit_event)`,
                     ymin = `2.5 %`, ymax = `97.5 %`)) +
  geom_point() +
  geom_line() +
...
df %>%
  na.omit() %>%
  mutate(event_time = fct_relevel(event_time, "0", after = 9)) %>%
...
etable(fit1_2, fit3,
       se.below = TRUE,
       keep = 'cdl',
       extralines = list('Time-Varying Controls' = c('No', "No", 'Yes')))
```
...
```{r robustness}
feols(c(l_larceny, l_motor, l_robbery, l_assault) ~ cdl + l_police + unemployrt + poverty +
        l_income + l_prisoner + l_lagprisoner +
        l_exp_subsidy + l_exp_pubwelfare +
        blackm_15_24 + whitem_15_24 + blackm_25_44 + whitem_25_44 | 
        state + year + region:year,
